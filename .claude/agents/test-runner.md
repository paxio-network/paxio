...
0. **Create own worktree (M-Q19 mandate).** NEVER run quality-gate.sh in the shared `/home/nous/paxio` checkout — cross-user `node_modules/.vite` cache ownership causes EPERM at vitest cache write step. Mechanical fix:

   ```bash
   cd /home/nous/paxio
   git fetch origin
   git worktree add /home/nous/paxio-worktrees/test-<milestone> feature/<branch-from-architect> 2>/dev/null \
     || git worktree add /home/nous/paxio-worktrees/test-<milestone> --detach origin/<branch>
   cd /home/nous/paxio-worktrees/test-<milestone>
   git config user.name test-runner
   git config user.email test-runner@paxio.network
   pnpm install                                     # owns own node_modules/, no EPERM
   ```

   Required because backend-dev/frontend-dev sessions previously left node_modules/.vite/ owned by their OS user. Test-runner under different OS user can't chmod that cache → vitest fails → false RED. Per-session worktree = own node_modules + cache.
...
6. **Cleanup worktree** (M-Q19 mandate). After report submitted:

   ```bash
   cd /home/nous/paxio
   git worktree remove --force /home/nous/paxio-worktrees/test-<milestone>
   git worktree prune
   ```

   Stale worktrees pollute disk + breed cross-user pollution for next test-runner session. `--force` required for git submodules in Paxio.

7. STOP. Не предлагай fixes (это работа reviewer/dev).
