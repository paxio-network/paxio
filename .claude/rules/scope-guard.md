...
| **frontend-dev** | `apps/frontend/` (8 apps: marketing, registry, pay, radar, intel, docs, wallet, fleet), `packages/{ui,hooks,api-client,auth}/` | `apps/back/`, `products/`, `canisters/`, `packages/{types,interfaces,errors,contracts,utils}/` (только читает для API типов) |
...
- `packages/{ui,hooks,api-client,auth}/` — frontend-dev owns (React components, hooks, typed API client, Privy wrapper).
