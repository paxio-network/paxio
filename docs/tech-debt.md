...
| TD-05 | `landing-stats.ts::nowIso` вызывает `new Date().toISOString()` внутри domain — impure (engineering-principles §6). Для testability и детерминизма вынести в `Clock` dep: `LandingStatsDeps.clock: () => number`, использовать `deps.clock()` в `nowIso`. | backend-dev | 🟢 LOW | `products/07-intelligence/tests/landing-td-fixes.test.ts` — 4 теста в `describe('TD-05: …')`. Сейчас 3 RED + 1 случайно GREEN. После fix: 4/4 GREEN. | 🔴 OPEN |
| TD-06 | M01c API handlers (7 штук) не валидируют query через Zod — `landing-agents-top.js` использует `typeof query?.limit === 'number'` без bounds-check. Должно быть Zod-схема с диапазоном (например `z.coerce.number().int().min(1).max(100)`) и явный `throw new errors.ValidationError(...)`. | backend-dev | 🟢 LOW | `products/07-intelligence/tests/landing-td-fixes.test.ts` — 4 теста в `describe('TD-06: …')`. Сейчас 3 RED (limit=-1, 999999, "20" string) + 1 GREEN (valid limit=20). После fix: 4/4 GREEN. | 🔴 OPEN |
