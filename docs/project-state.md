...
**Last Commit:** `bf8176f` (M01c Landing + TD-01 fix + TD-08/TD-09 attempts reviewed — 6 new tech debts TD-11..TD-16 recorded; APPROVED WITH DEBT per user decision to defer fixes)
...
**Outstanding tech debt:** 18 entries after 2026-04-23 M01c Landing review.
- **OPEN HIGH (4)**: TD-11 verify_m01c_landing.sh broken (architect), TD-12 Real Data Invariant violations (frontend-dev), TD-13 5th architect→dev scope recurrence (governance), TD-14 TD-01 fix over-engineered (backend-dev).
- **BACKLOG MED (3)**: TD-01 errors sync (test landed, fix TD-14), TD-08 smoke tests partial-fix (1/8 apps, frontend-dev), TD-09 ESLint broken-fix attempt (`eslint-config-next` missing, frontend-dev).
- **BACKLOG LOW (2)**: TD-15 eval() in test (backend-dev), TD-16 @paxio/ui pure-fn tests missing (frontend-dev).
- **ACK (7)**: TD-02/TD-03/TD-04/TD-07/TD-10/TD-M01-1/TD-M01-2 (governance notes + future-work placeholders).
- **✅ CLOSED (2)**: TD-05/TD-06 (Clock DI + Zod bounds in M01c backend).
