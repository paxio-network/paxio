...
0. **Create own worktree (M-Q19 mandate).** NEVER run quality-gate.sh in the shared `/home/nous/paxio` checkout — cross-user `node_modules/.vite` cache ownership causes EPERM at vitest cache write step. Mechanical fix:
   ...
   cd /home/nous/paxio
   git fetch origin
   git worktree add /tmp/paxio-test-<milestone> feature/<branch-from-architect> 2>/dev/null \
     || git worktree add /tmp/paxio-test-<milestone> --detach origin/<branch>
   cd /tmp/paxio-test-<milestone>
...
6. **Cleanup worktree** (M-Q19 mandate). After report submitted:
   ...
   ```bash
   cd /home/nous/paxio
   git worktree remove --force /tmp/paxio-test-<milestone>
   git worktree prune
   ```
