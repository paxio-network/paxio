...
**Last Commit:** `4bcc3cd` on `feature/td-close-backend-round` (TD-11 + TD-14 closed — `scripts/verify_m01c_landing.sh` mkdir fix, `apps/back/server/lib/errors.cjs` inline `Object.freeze`'d constants replacing dist/ require; drift-guard 13/13 GREEN + verify-script robustness 20/20 GREEN; reviewer APPROVED, ready for PR → dev)
...
| M01c: Landing frontend (FA-10 partial) | 🟡 PARTIAL-WITH-DEBT (architect-authored) | `apps/frontend/landing/app/{page.tsx, providers.tsx, sections/01-hero.tsx..06-doors.tsx}` + `packages/ui/src/{AgentTable, FAPDiagram, Footer, HeatmapGrid, LiveTicker, NetworkGraph, SectionFrame, Sparkline, TerminalWidget}.tsx` + `packages/hooks/src/{useAgent, useGuard, useTicker, useWallet}.ts` + `packages/api-client/src/index.ts`. Landing app builds clean (Next.js 15 `✓ Compiled successfully in 1072ms`, 4 static pages). Written by architect (commit `bf8176f`) — frontend-dev scope, 5-й повтор TD-07 pattern → TD-13 OPEN escalation. **Outstanding debts (all deferred per user)**: TD-12 Real Data Invariant violations в 04-pay (`DEFAULT_RAILS` hardcoded) + 05-network (`emptySnapshot()` 20 fake Agent-XX nodes) 🔴; TD-16 `@paxio/ui` pure functions без unit tests 🟡. |
...
**Outstanding tech debt:** 18 entries after 2026-04-23 TD-close backend round (TD-11 + TD-14 closed).
- **OPEN HIGH (2)**: TD-12 Real Data Invariant violations (frontend-dev — specs в `tests/_specs/real-data-invariant.test.ts` 11 RED), TD-13 5th architect→dev scope recurrence (governance).
- **BACKLOG MED (3)**: TD-01 errors sync (inline constants + drift-guard landed, keep monitoring), TD-08 smoke tests partial-fix (1/8 apps, frontend-dev), TD-09 ESLint broken-fix attempt (`eslint-config-next` missing, frontend-dev).
- **BACKLOG LOW (2)**: TD-15 eval() in `tests/errors-cjs-sync.test.ts` (architect scope — test file; spec `tests/_specs/no-eval-in-tests.test.ts` 1 RED; unblocked by TD-14 closure), TD-16 @paxio/ui pure-fn tests missing (frontend-dev — specs 12 RED).
- **ACK (7)**: TD-02/TD-03/TD-04/TD-07/TD-10/TD-M01-1/TD-M01-2 (governance notes + future-work placeholders).
- **✅ CLOSED (4)**: TD-05/TD-06 (Clock DI + Zod bounds in M01c backend); TD-11 (`347f1a4` — mkdir `$HOME/tmp` в verify_m01c_landing.sh + 20-assert meta-drift-guard); TD-14 (`4bcc3cd` — errors.cjs inline constants, no dist/ require, server boots without `pnpm build`).
