---
description: "[ARCHIVED M-Q13] Backend architecture — apps/back server/app separation, VM sandbox, layered loading — moved to skill `paxio-backend-architecture`. Kept for git history; no longer auto-loads."
globs: []
---

> **ARCHIVED in M-Q13** — content ported to skill `.claude/skills/paxio-backend-architecture/SKILL.md`.
...
|---|---|---|
| `agentDid` | `session.agentDid` (выставлен auth middleware из подписанного DID-токена) | Wallet balance, transactions, signed intents, registry claims, FAP routes для конкретного агента |
| `organizationId` | `session.organizationId` (для fleet/enterprise — set по WorkOS/SSO) | Fleet dashboard, batch операции, биллинг, audit feed для всей организации |
...
// ❌ НЕПРАВИЛЬНО — identity из body клиент может подделать = data leak / impersonation
method: async ({ body }) => {
  return await db.query(
    'SELECT * FROM transactions WHERE agent_did = $1',
    [body.agentDid]   // КЛИЕНТ ПОДСТАВИЛ ЧУЖОЙ DID
  );
}
```
...
// ✅ fleet view — фильтр по organizationId
const fleetAgents = await db.query(
...
Эти данные **НЕ фильтруются** — они публичные by design:
...
Любой другой endpoint, который ХОЧЕШЬ сделать публичным — `!!! SCOPE VIOLATION REQUEST !!!`. Architect добавит в whitelist явно.

### Inter-canister calls — identity передаётся
...
### Frontend NEVER trusts identity from URL/body

```tsx
// ✅ frontend получает свой DID через @paxio/auth (из session)
const did = useDid();   // 'did:paxio:0x...' — из подписанной сессии

// ❌ НЕПРАВИЛЬНО — DID из URL или localStorage
const did = searchParams.get('agentDid');   // user может подменить
```

### Reviewer P0 check (B1-B7)

Reviewer Phase 2 (см. `.claude/agents/reviewer.md`) — **первая** проверка после scope:
...
Любой fail → **REJECT, severity=CRITICAL** в `tech-debt.md`. Это data leak / финансовый риск.
